...
- 评测任务列表：展示任务状态、目标模型、端点数量、样本数量和创建时间。
- 任务创建器：选择模型、候选端点、手工维护的样本集、评审端点和评审模型。
- 结果总览：以端点为行展示成功率、TTFT、总耗时、Token、百分制效果评分和排名。
...
评测任务创建后进入队列。执行器以串行方式将每条样本向每个候选端点发送三次，并直接指定端点，绕开常规请求调度。单次调用写入原始性能结果和聚合后的流式文本。全部被测调用完成后，自动评审器以同一份评分规则评估每个输出。汇总接口按照端点、样本和重复次数返回结果。

## 数据模型草案

- `ModelBenchmarkRun`：任务定义、状态、创建时间、目标模型、候选端点、评审配置和固定重复次数三次。
- `ModelBenchmarkCase`：任务中的单条名称、输入消息、可选系统提示词和评测参数。
- `ModelBenchmarkAttempt`：端点、样本、重复序号、状态码、TTFT、总耗时、Token、输出、错误信息和截断标记；输出上限为 1MB。
- `ModelBenchmarkJudgeResult`：尝试结果标识、总分、准确性分、完整性分、指令遵循分、表达质量分、评分理由、置信度和评审状态；评审响应上限为 1MB。
- `ModelBenchmarkSummary`：按端点聚合的成功率、TTFT 中位数、总耗时中位数、平均 Token、平均总分和各维度平均分。
...
- 同一任务内，所有候选端点使用相同模型名称、样本内容和请求参数。
- 每个候选端点与样本组合恰好创建三条被测尝试记录，执行器以串行方式调度尝试，取消前已开始的尝试保留实际结果。
- 单个端点或评审调用失败只影响关联尝试，任务汇总继续计算其他已完成结果。
- 端点汇总只使用完成状态的性能数值；成功率以该端点所有三次尝试为分母。
