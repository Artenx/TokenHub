...
管理后台将技能仓库作为一级导航，提供本地技能、联网搜索和来源设置三个视图。上传 ZIP、公开来源归档和链接归档都先请求预览接口，用户确认后才导入；同名包显示替换操作，删除操作要求输入目录名确认。搜索页面同时展示成功来源和来源级失败状态，来源设置提供 GitHub、SkillHub 预置项及 HTTPS 自定义索引。
